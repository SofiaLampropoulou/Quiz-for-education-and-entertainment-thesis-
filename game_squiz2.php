...
<script>
    ...
    function showex(pageNr = 0){
        $.getJSON("api.php?c=studentquiz&id=" + id, (res) => {
            ...
            cor = res;
            totalPages = cor.length; // Set total pages to number of exercises
            ...
            if (cor[pageNr]) {
                ...
                for (let j = 0; j < cor[pageNr]["a"].length; j++) {
                    html1 += ` <p><input type="radio" name="aa${ cor[pageNr]["q"].id }" value='${ cor[pageNr]["a"][j].id }'> ${ cor[pageNr]["a"][j].text_of_answer }</p>`;
                }
                html1 += "</div>";
                $("#sgamequiz").append(html1);
            }

            // Show/Hide Next button depending on the page
